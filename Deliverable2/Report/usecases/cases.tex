...
- Main Scenarios
    ...
    (d) The controller notifies the robot that his move request has been approved and that the robot has been moved to the requested position
    (e) The controller notifies the viewer that there are changes and sends the snapshot along with this
- Alternatives
    (d.1) The controller notifies the viewer that there are changes and sends the snapshot along with this
    (e.1) The controller notifies the robot that his move request has been approved and that the robot has been moved to the requested position
...
- Main Scenarios
    ...
    (c) The chosen tiles are exchanged (ordinary exchange)
    (d) The board returns an empty robot pair to the controller
    (e) The board makes a snapshot and sends this to the controller
    (f) The controller notifies the viewer that there are changes and sends the snapshot along with this
- Alternatives
    (e.1) If a chosen tile contains a robot, the robot will rotate (this can be the same) and swaps along with the tile (special exchange)
    (f.1) The board returns a robot pair containing the switched robot
    (g.1) The board notifies the controller that a robot has moved
    (h.1) The controller notifies the robot that is has been moved
    (i.1) The board makes a snapshot and sends this to the controller
    (j.1) The controller notifies the viewer that there are changes and sends the snapshot along with this

    (e.2) If a chosen tile is a conveyor tile, the rotation of the conveyor tile might change (can also be the same) (special exchange)
    (f.2) The board makes a snapshot and sends this to the controller
    (g.2) The controller notifies the viewer that there are changes and sends the snapshot along with this
